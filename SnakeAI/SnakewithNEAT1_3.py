...
moving_left = {"left": (0,  unit_velocity,3),"right":(0, -unit_velocity,2),"up":(-unit_velocity, 0,0),"down":(unit_velocity,  0,1)}
moving_right = {"left": (0,  -unit_velocity,2),"right":(0, unit_velocity,3),"up":(unit_velocity, 0,1),"down":(-unit_velocity,  0,0)}
moving_up = {"left": (-unit_velocity,   0,0),"right":(unit_velocity, 0,1),"up":(0,  unit_velocity,2),"down":(0,   -unit_velocity,3)}
moving_down = {"left": (unit_velocity,  0,1),"right":(-unit_velocity, 0,0),"up":(0,    -unit_velocity,3),"down":(0,  unit_velocity,2)}

orientations = [moving_left,moving_right,moving_up,moving_down]
pygame.init()
...
class Snake:
    ...
    def get_body_vectors(self):
        ...
        if self.length>1:
            for i in self.body:
                body_dirn.append((i[0]-self.x, i[1]-self.y))
        else:
            body_dirn = [(0,0),(0,0)]
        return  body_dirn
    def motion(self):
    ...
    def changedir(self,dirn):
        my_FOR = orientations[self.orientation]
        self.x_vel = my_FOR[dirn][0]
        self.y_vel = my_FOR[dirn][1]
        self.orientation = my_FOR[dirn][2]
        # if dirn =="left":
        #     self.x_vel = -unit_velocity
        #     self.y_vel = 0
        # elif dirn =="right":
...
def playGame(genomes, config):
    ...
    while gameLoop:
        ...
        for x, mySnakeHead in enumerate(mySnakeHeads):
            ...
            body_dirn = currMySnakeHeads.get_body_vectors()
            netInput =( (currMyFruits.x-currMySnakeHeads.x)/display_width,
                        (currMyFruits.y-currMySnakeHeads.y)/display_height,
                        currMySnakeHeads.x_vel/unit_velocity,
                        currMySnakeHeads.y_vel/unit_velocity,
                        currMySnakeHeads.x/display_width,
                        currMySnakeHeads.y/display_height,
                        (display_width - currMySnakeHeads.x)/display_width,
                        (display_height - currMySnakeHeads.y)/display_height,
                        body_dirn[0][0]/(2*block_size),
                        body_dirn[0][1]/(2*block_size),
                        body_dirn[1][0]/(2*block_size),
                        body_dirn[1][1]/(2*block_size) )
            ...
            if currMySnakeHeads.selfCollide():
                ...
                currGenomeList.fitness += 10
                currMySnakeHeads.increase_length()
                currStats.timeout = 0
                currStats.score+=1    
            
            elif currStats.timeout >= FPS*5:
