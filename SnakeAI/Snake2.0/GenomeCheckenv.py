...
def playGame(genome, config):
    ...
    while True: # Ultra Game Loop
        score = 0
        currMySnakeHead = Snake(200,200)
        # snakeList = [mySnakeHead]
        currMyFruits = Fruits(random.randint(0,display_width-fruit_block_size),random.randint(0,display_height-fruit_block_size))     
        while gameLoop:
            clock.tick(FPS)
            print(currMySnakeHead.vision())
            ...
            netInput =(currMyFruits.x-currMySnakeHead.x,
                        currMyFruits.y-currMySnakeHead.y,
